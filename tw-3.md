...
- **可觀察性勝於不透明性。** 透過決策追蹤、來源引用、不確定性分數，以及繫於決策的可解釋摘要「展示你的工作」。（可觀察性意味著系統的推論過程是可供檢驗的，而不是讓任何人看見個人的隱私互動；隱私與透明並不衝突。）
...
3. **運行影子模式。** 新政策看到輸入並提議行動但不行動。與人類/先前系統比較。影子模式，就是學徒站在師傅肩後觀摩。
...
輔助診斷包括防護措施完整性、決策軌跡完整度、金絲雀健康度，以及稽核推翻率。唯有當獨立稽核員能重建決策軌跡、金絲雀切片真正具有代表性時，這個比率才會上升——絕不會是系統在簡單案例上過關、再把儀式當作證明的時候。詳見[衡量指標](/tw/measures/)。
