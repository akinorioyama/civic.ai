...
- **憲制式有界。** 每個智慧體都有目的界限、資源上限，以及事先載明的退場期限。退場的是*元件*；持續的是*服務義務*——繼任計畫確保總有人承接這份責任。
...
受託守護也有經濟上的一面。當地神汲取社群的知識——一位長者的脈絡、一位譯者的細膩語感、一個工藝合作社的手藝——它本來就保存的決策軌跡，便同時是一張公民收據（civic receipt）：其設計是向預先注資的託管帳戶結算，讓價值回流到社群的[資料聯盟](/tw/glossary/#data-coalition)，而不是被抽取出去。這套機制的軌跡那一半今天已經在運行；結算那一半，就我們所知還沒有在任何地方上路。價值回流到孕育它的土壤。
...
- **後設角色。** Pack 6 不取代 Packs 1 到 5；它防止前五力走向自身的反面：沒有界限的覺察會變成監控；負責會變成契約陷阱；勝任會變成不可或缺；回應會變成假扮關懷的依賴；團結則會變成無人控制、人人依賴的基礎設施。
