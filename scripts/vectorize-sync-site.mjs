#!/usr/bin/env node
/**
 * Upsert Civic AI site search corpus into Vectorize `civic-ai-site` (bge-m3, 1024-dim).
 *   bun scripts/vectorize-sync-site.mjs   # uses wrangler login + wrangler auth token when unset
 *   CLOUDFLARE_ACCOUNT_ID=… CLOUDFLARE_API_TOKEN=… bun scripts/vectorize-sync-site.mjs
 *   LIMIT=50 DRY_RUN=1 bun scripts/vectorize-sync-site.mjs
 *
 * Prerequisite (from repo root, worker package):
 *   cd worker && bunx wrangler vectorize create civic-ai-site --dimensions=1024 --metric=cosine
 * Metadata index lang BEFORE bulk upsert (not retroactive):
 *   cd worker && bunx wrangler vectorize create-metadata-index civic-ai-site --propertyName=lang --type=string
...
import { getSearchEntriesByLang } from "../src/lib/search.ts";
...
const INDEX = process.env.VECTORIZE_INDEX ?? "civic-ai-site";
const WRANGLER_CWD = path.join(ROOT, "worker");
const LIMIT = process.env.LIMIT ? Number(process.env.LIMIT) : Infinity;
const DRY_RUN = process.env.DRY_RUN === "1";
const BATCH = Math.min(
    32,
    Math.max(1, Number(process.env.EMBED_BATCH ?? "16"))
);
...
function resolveAccountId() {
    ...
    const json = execSync("bunx wrangler whoami --json", {
        encoding: "utf8",
        cwd: WRANGLER_CWD,
...
function resolveApiToken() {
    ...
    const output = execSync("bunx wrangler auth token 2>/dev/null", {
        encoding: "utf8",
        cwd: WRANGLER_CWD,
    });
    ...
    if (!token) {
        throw new Error(
            "No Cloudflare API token. Run `wrangler login` or set CLOUDFLARE_API_TOKEN."
        );
    }
    return token;
}
...
async function embedTexts(accountId, apiToken, texts) {
    ...
    if (!res.ok)
        throw new Error(
            `embed HTTP ${res.status}: ${(await res.text()).slice(0, 300)}`
        );
...
function upsertNdjson(filePath) {
    const cmd =
        `bunx wrangler vectorize upsert ${shellQuote(INDEX)} ` +
        `--file ${shellQuote(filePath)} --batch-size 100`;
    execSync(cmd, {
        cwd: WRANGLER_CWD,
        env: buildWranglerEnvForUpsert(),
        stdio: "inherit",
    });
}
...
function deleteVectors(ids) {
    ...
    for (let i = 0; i < ids.length; i += 100) {
        ...
        const cmd = `bunx wrangler vectorize delete-vectors ${shellQuote(INDEX)} --ids ${quotedIds}`;
        execSync(cmd, {
            cwd: WRANGLER_CWD,
            env: buildWranglerEnvForUpsert(),
            stdio: "inherit",
...
async function ensureLangMetadataIndex() {
    ...
    const listCmd = `bunx wrangler vectorize list-metadata-index ${shellQuote(INDEX)}`;
    const createCmd = `bunx wrangler vectorize create-metadata-index ${shellQuote(INDEX)} --propertyName=lang --type=string`;
    const wranglerOpts = {
        cwd: WRANGLER_CWD,
        env: buildWranglerEnvForUpsert(),
    };
    const langIndexListed = (out) =>
        /\blang\b/.test(out) && /String/i.test(out);
    try {
    ...
    console.log(
        "creating metadata index lang (required for worker lang filter)…"
    );
    ...
    throw new Error(
        "metadata index lang not ready after wait; retry sync later"
    );
}

async function main() {
    const indexByLang = getSearchEntriesByLang();
    ...
    if (DRY_RUN) {
        console.log(
            "[dry-run] generated records only; no Cloudflare credentials, metadata indexes, embeddings, deletes, or upserts used"
        );
        return;
    }
    ...
    const staleIds = Array.from(
        new Set(records.map((r) => r.replacesId).filter(Boolean))
    );
    ...
    for (let i = 0; i < records.length; i += BATCH) {
        ...
        if (DRY_RUN) {
            ...
            upsertNdjson(ndjson);
            console.log(
                `upserted ${(done += slice.length)} / ${records.length}`
            );
        }
    }
